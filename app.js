/**
 * Specification Manager - Main Application Logic
 * 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const specSetSelector = document.getElementById('specSetSelector');
    ...
    let currentSpecSet = 'ladotd-2016'; // Default spec set
    ...
    async function initApp() {
        ...
        specSetSelector.addEventListener('change', handleSpecSetChange);
        
        // Load available spec sets and initialize with default
        try {
            // Initialize with default spec set
            await SpecificationMatcher.init();
            console.log('Specification data loaded successfully');
            
            // Set initial value of spec set selector
            specSetSelector.value = SpecificationMatcher.getCurrentSpecSet();
        } catch (error) {
    ...
    // Handle specification set change
    function handleSpecSetChange() {
        const newSpecSet = specSetSelector.value;
        console.log(`Changing specification set to: ${newSpecSet}`);
        
        // Update the spec matcher
        SpecificationMatcher.setSpecSet(newSpecSet);
        currentSpecSet = newSpecSet;
        
        // Clear any processed data to avoid mixing specs
        processedItems = [];
